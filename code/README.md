...
Contém os **dados globais** compartilhados entre os clientes e as **funções principais** do algoritmo.
...
Implementa o **algoritmo de segurança**. Verifica se o estado atual do sistema é seguro, ou seja, se existe ao menos uma sequência em que todos os clientes conseguem terminar.
...
### `solicitar_recursos(cliente_id, solicitacao) -> int`

Processa uma solicitação de recursos de um cliente. Retorna `0` se aprovada, `-1` se negada.
...
### `liberar_recursos(numero_cliente, liberacao) → int`
 
Processa a devolução de recursos por um cliente. Retorna `0` se bem-sucedida, `-1` em caso de erro.
...
Define a **lógica de cada thread de cliente** e a função que cria e gerencia todas as threads.

---
 
### `_rotina(cliente_id) -> None`
 
Função executada por cada thread. Roda um loop de `NUMERO_DE_CICLOS` iterações onde cada ciclo consiste em:
...
### `iniciar_clientes() -> None`
 
Cria todas as threads de clientes, inicia-as de forma concorrente e aguarda o término de todas.
...
Ponto de entrada do programa. Responsável por unir as partes e inicializar a execução.

---
 
### `_inicializar() -> None`
 
Inicializa as estruturas de dados globais do banqueiro.
...
Chama `_inicializar()` para configurar o ambiente e depois `iniciar_clientes()` para começar a simulação.
